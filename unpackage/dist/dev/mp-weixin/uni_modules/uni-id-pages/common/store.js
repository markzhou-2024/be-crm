  ...
  async updateUserInfo(data2 = false) {
    if (data2) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at uni_modules/uni-id-pages/common/store.js:57", e.message, e.errCode);
      }
    }
  },
  ...
  async logout() {
    if (common_vendor.tr.getCurrentUserInfo().tokenExpired > Date.now()) {
      try {
        ...
        common_vendor.index.__f__("error", "at uni_modules/uni-id-pages/common/store.js:76", e);
      }
    }
      ...
      return common_vendor.index.redirectTo({
        ...
        fail: (err1) => {
          common_vendor.index.switchTab({
            url: uniIdRedirectUrl,
            fail: (err2) => {
              common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/common/store.js:106", err1, err2);
            }
          });
        }
      });
  ...
  loginSuccess(e = {}) {
    ...
    if (uni_modules_uniIdPages_config.config.setPasswordAfterLogin && !passwordConfirmed) {
      return common_vendor.index.redirectTo({
        ...
        fail: (err) => {
          common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/common/store.js:150", err);
        }
      });
    }
    if (autoBack) {
